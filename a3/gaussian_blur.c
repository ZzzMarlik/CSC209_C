...
void gaussian_blur_filter(Bitmap *bmp) {
    ...
    while (col < bmp->width - 1) { // handle the first row and second row
        if (col == 0) { // handle (0, 0)
            Pixel new_p = apply_gaussian_kernel(&(top[col]), &(mid[col]), &(bot[col]));// get new (1, 1)
            new_ps[col] = new_p;
            new_ps[col + 1] = new_p;
            col += 2;
            int e1, e2;
            ...
            Pixel new_p = apply_gaussian_kernel(&(top[col - 1]), &(mid[col - 1]), &(bot[col - 1])); //get new (1, width - 2)
            new_ps[col] = new_p;
            new_ps[col + 1] = new_p;
            col += 2;
            int e1, e2;
            ...
            Pixel new_p = apply_gaussian_kernel(&(top[col - 1]), &(mid[col - 1]), &(bot[col - 1]));
            new_ps[col] = new_p;
            col += 1;
    ...
    while (row < bmp->height - 2){ // middle cases
        ...
        while (col < bmp->width - 1) {
            if (col == 0) {
                Pixel new_p = apply_gaussian_kernel(&(top[col]), &(mid[col]), &(bot[col]));
                col += 2;
                int e1, e2;
